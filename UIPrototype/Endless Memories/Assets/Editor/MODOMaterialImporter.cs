...
public class MODOMaterialImporter : AssetPostprocessor
{
	...
	*/
	...
	public static bool parseNumericValue(string input, float[] output, out int outputCount)
	{
		...
		{
			success = float.TryParse(elements[e], NumberStyles.Any, CultureInfo.InvariantCulture, out output[e]);
			if (!success)
			{
				break;
			}
	...
	*/
	...
	Material OnAssignMaterialModel(Material material, Renderer renderer)
	{
		...
		string xmlPath = NormalizePath(Path.Combine(Path.GetDirectoryName(assetPath), Path.GetFileNameWithoutExtension(assetPath) + "_mat.xml"));
		MODOMaterialContainer matContainer = LoadMaterialXMLFromPath(xmlPath);

		// If the path with _mat can't be found then search for XML without the suffix.
		if (matContainer == null)
		{
			xmlPath = NormalizePath(Path.Combine(Path.GetDirectoryName(assetPath), Path.GetFileNameWithoutExtension(assetPath) + ".xml"));
			matContainer = LoadMaterialXMLFromPath(xmlPath);
		}
		...
		{
			DebugWarning("No FBX file \"" + modelPath + "\" for XML \"" + xmlPath + "\" found.");
		}
		...
		foreach (var str in importedAssets)
		{
			if (Path.GetExtension(str) == ".xml")
			{
				...
				if (matContainer != null)
				{
					...
					{
						bool exists = false;
						if (str.EndsWith("_mat.xml"))
						{
							string modelPathReduced = Path.GetFileNameWithoutExtension(str);
							modelPathReduced = modelPathReduced.Remove(modelPathReduced.Length - 4);
							modelPathReduced = Path.Combine(Path.GetDirectoryName(str), modelPathReduced + ".fbx");
							if (AssetDatabase.LoadAssetAtPath(modelPathReduced, typeof(GameObject)) != null)
							{
								AssetDatabase.ImportAsset(modelPathReduced, ImportAssetOptions.ForceUpdate);
								DebugLog("Reimporting FBX file for XML.");
								exists = true;
							}
						}

						if (AssetDatabase.LoadAssetAtPath(modelPath, typeof(GameObject)) != null && !exists)
						{
							...
							if (matProp != null)
							{
									...
									)
								{
									...
									if (param.name == "Albedo")
									{
										...
										EditorGUILayout.ColorField(new GUIContent("Albedo Color"), mat.getDiffuseColor(), false, false, false);
#else
